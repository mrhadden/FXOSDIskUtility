...
final class Fat
{
	 ...
	 *             if {@code fatNr} is greater than {@link BootSector#getNrFats()}
	 */
	public static Fat read(BootSector bs, int fatNr) throws IOException, IllegalArgumentException
	{
		...
		if (fatNr > bs.getNrFats())
		{
			throw new IllegalArgumentException(
					"boot sector says there are only " + bs.getNrFats() + " FATs when reading FAT #" + fatNr);
		}
	...
	public static Fat create(BootSector bs, int fatNr) throws IOException, IllegalArgumentException
	{
		...
		{
			throw new IllegalArgumentException(
					"boot sector says there are only " + bs.getNrFats() + " FATs when creating FAT #" + fatNr);
		}
	...
	private Fat(BootSector bs, long offset) throws IOException
	{
		...
		if (lastClusterIndex > entries.length)
			throw new IOException(
					"file system has " + lastClusterIndex + "clusters but only " + entries.length + " FAT entries");
	}
	 ...
	 * This estimate does only account for clusters that are really available in the
	 * data portion of the file system, not for clusters that might only
	 ...
	 * 
	 * @param cluster
	 *            a cluster from a chain where the new cluster should be appended
