...
final class FatDirectoryEntry extends AbstractFsObject
{
	...
	/**
	 * Reads a {@code FatDirectoryEntry} from the specified {@code ByteBuffer}. The
	 * buffer must have at least {@link #SIZE} bytes remaining. The entry is read
	 * from the buffer's current position, and if this method returns non-null the
	 * position will have advanced by {@link #SIZE} bytes, otherwise the position
	 ...
	 * @return the directory entry that was read from the buffer or {@code null} if
	 *         there was no entry to read from the specified position (first byte
	 *         was 0)
	 */
	...
	{
		return DosUtils.decodeDateTime(LittleEndian.getUInt16(data, 0x12), 0); /* time is not recorded */
	}
	...
	/**
	 * Returns if the read-only flag is set for this entry. Do not confuse this with
	 * {@link #isReadOnly()}.
	 *
