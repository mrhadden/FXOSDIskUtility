...
abstract class AbstractDirectory
{
	 ...
	 * @param isRoot
	 *            if the new {@code AbstractDirectory} represents a root directory
	 */
	...
	/**
	 * Gets called when the {@code AbstractDirectory} must read it's content off the
	 * backing storage. This method must always fill the buffer's remaining space
	 * with the bytes making up this directory, beginning with the first byte.
	 *
	 * @param data
	...
	/**
	 * Gets called when the {@code AbstractDirectory} wants to write it's contents
	 * to the backing storage. This method is expected to write the buffer's
	 * remaining data to the storage, beginning with the first byte.
	 *
	 * @param data
	 ...
	 * important when creating the ".." entry in a sub-directory, as this entry must
	 * poing to the storage cluster of it's parent.
	 *
	...
	/**
	 * Gets called by the {@code AbstractDirectory} when it has determined that it
	 * should resize because the number of entries has changed.
	 *
	 * @param entryCount
	 ...
	 *
	 * @return the number of entries this directory can hold in its current storage
	 *         space
	 * @see #changeSize(int)
	 */
	...
	/**
	 * Gets the number of directory entries in this directory. This is the number of
	 * "real" entries in this directory, possibly plus one if a volume label is set.
	 * 
	...
	public void flush() throws IOException
	{
		...
		final ByteBuffer data = ByteBuffer
				.allocate(getCapacity() * FatDirectoryEntry.SIZE + (volumeLabel != null ? FatDirectoryEntry.SIZE : 0));
	...
	/**
	 * Returns the volume label that is stored in this directory. Reading the volume
	 * label is only supported for the root directory.
	 *
	...
	/**
	 * Sets the volume label that is stored in this directory. Setting the volume
	 * label is supported on the root directory only.
	 *
	 * @param label
