...
/**
 * Stores information about one directory of a virtual disk, and the contents of
 * that directory (subdirectories and files).
 *
...
public class VirtualDirectory extends VirtualFile
{
	...
	public List<VirtualFile> getContents()
	{
		...
		this.virtualFiles.stream().filter(e -> e.isDirectory()).sorted().forEach(e -> contents.add(e));

		// Then add all file entries, sorted alphabetically
		this.virtualFiles.stream().filter(e -> e.isFile()).sorted().forEach(e -> contents.add(e));
		return contents;
	}
	...
	{
		return this.virtualFiles.stream().filter(f -> f.getName().equals(name)).findFirst().orElse(null);
	}

	/**
	 * Returns the total size of this directories' contents.
	 *
	 * @return The number of bytes of all files in this directory, and
	...
	/**
	 * Recursively sums up the length of all files in this directory and all
	 * subdirectories.
	 *
	 * @param directory
	 *            The directory to check for content size.
	 * @param currentCount
	 ...
	 *
	 * @param newVirtualFile
	 *            The new file to add to this directory.
	 * @return the entry pointing to the new file
	 */
